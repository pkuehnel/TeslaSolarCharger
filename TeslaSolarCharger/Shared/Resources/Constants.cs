...
public class Constants : IConstants
{
    ...
    // Solar Energy (Yellow/Gold family - represents sun/generation)
    public string SolarPowerColor => "#FFA500";           // Orange (actual)
    public string SolarPowerPredictionColor => "#FFD700"; // Gold (prediction - lighter)

    // Grid (Blue/Purple family - represents utility/external)
    public string GridColor => "#4169E1";          // Royal Blue (base/lines)
    public string GridExportColor => "#32CD32";    // Lime Green (selling/positive)
    public string GridImportColor => "#4169E1";    // Royal Blue (buying/negative)

    // Battery (Teal/Cyan family - represents storage)
    public string BatteryColor => "#008B8B";       // Dark Cyan (base/lines)
    public string BatteryChargingColor => "#20B2AA";   // Light Sea Green (charging/storing)
    public string BatteryDischargingColor => "#FF6347"; // Tomato (discharging/using)
    public string BatterySocColor => "#2F4F4F";    // Dark Slate Gray (SOC text)

    // Home Consumption (Purple/Magenta family - represents usage)
    public string HomeConsumptionColor => "#9370DB";       // Medium Purple (base)
    public string HomeConsumptionChartColor => "#8A2BE2";  // Blue Violet (actual in charts)
    public string HomeConsumptionPredictionColor => "#DDA0DD"; // Plum (prediction - lighter)

    // EV Charging (Gray/Silver family - represents vehicles)
    public string EvChargingColor => "#708090";    // Slate Gray (more visible than light grey)

    // Flow indicators (for the SVG component)
    public string ConsumptionColor => "#DC143C";   // Crimson (negative flow)
    public string FeedInColor => "#32CD32";        // Lime Green (positive flow)
