    ...
    : IBackendApiService
{
    ...
    public async Task<DtoCarLicenseInfo> GetFleetApiLicenseInfo()
    {
        ...
        var result = await SendRequestToBackend<DtoCarLicenseInfo>(HttpMethod.Get, token.AccessToken, "Client/GetFleetApiLicenseInfo", null).ConfigureAwait(false);
        if (result.HasError)
        {
            throw new InvalidOperationException(result.ErrorMessage);
        }
        return result.Data ?? new DtoCarLicenseInfo();
    }
    ...
    public async Task<Result<T>> SendRequestToBackend<T>(HttpMethod httpMethod, string? accessToken, string requestUrlPart, object? content)
    {
        ...
        {
            request.Headers.AcceptLanguage.Add(new System.Net.Http.Headers.StringWithQualityHeaderValue(fallbackCulture, 0.9));
        }
