...
public static class WorkerResponseMapper
{
    ...
    public static DtoBleCommandResult ToCommandResult(WorkerResponse response)
    {
        ...
        //No scan block any more: the worker does not look for the car before dialing, because whether the car is
        //around at all is decided here from the background scan's stream.
        if (response.Ok)
        {
    ...
    /// <summary>
    /// Whether a parsed line is the answer to the request currently in flight. Everything else - the unsolicited
    /// advertisement digests and scanner state lines of the background scan, and any stale answer - must never be
    /// mistaken for it, which is the one way the shared stdout pipe could desynchronize.
    /// </summary>
    public static bool IsResponseTo(WorkerResponse? parsed, int pendingRequestId) =>
        pendingRequestId == 0
            ? parsed?.Kind is "ready" or "fatal"
            : parsed?.Kind == "result" && parsed.Id == pendingRequestId;

    /// <summary>
    /// Whether a parsed line is one of the background scan's unsolicited events, which are routed to the presence
    /// registry instead of to a waiting caller.
    /// </summary>
    public static bool IsScanEvent(WorkerResponse? parsed) => parsed?.Kind is "adv" or "scan";
